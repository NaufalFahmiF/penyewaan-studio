...
if ($_GET['aksi'] == 'tambah') {
    ...
    if(!in_array($filetype, array_keys($allowedTypes))) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $sql = "INSERT INTO `studio`(`no_studio`, `jenis`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
            $row = $koneksi->prepare($sql);
            $row->execute($data);
            echo '<script>alert("sukses");window.location="studio.php"</script>';
        } else {
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE studio SET no_studio= ?, jenis=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_studio = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
...
if (!empty($_GET['aksi'] == 'hapus')) {
    ...
    echo '<script>alert("sukses hapus");window.location="studio.php"</script>';
}
